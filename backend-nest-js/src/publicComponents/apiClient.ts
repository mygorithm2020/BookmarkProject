...
export class ApiClient {
  ...
  async setSiteParse(reqUrl: string): Promise<Site> {
    ...
    if (response) {
      try {
        ...
        const charset =
          contentType && contentType.toLowerCase().includes('charset=')
        ...
        for (let idx = 0; idx < metas.length; idx++) {
          if (
            metas[idx].getAttribute('http-equiv') &&
            metas[idx].getAttribute('http-equiv').toLowerCase() ===
              'content-type' &&
            ...
            metas[idx].getAttribute('charset') &&
            metas[idx].getAttribute('charset').toLowerCase() === 'euc-kr'
          ) {
            root = Parse(iconv.decode(response.data, 'euc-kr'));
            break;
          }
        ...
        for (let idx = 0; idx < links.length; idx++) {
          if (links[idx].getAttribute('rel')) {
            ...
            if (linkRel === 'shortcut icon' || linkRel === 'icon') {
              const size = links[idx].getAttribute('sizes')
                ? parseInt(links[idx].getAttribute('sizes'))
                : 0;
        ...
        const metaEl = root.querySelectorAll('meta');
        for (let idx = 0; idx < metaEl.length; idx++) {
          ...
          if (metaEl[idx].getAttribute('name')) {
            ...
            const metaFromProperty = metaEl[idx]
  ...
  async setSiteJSDOM(reqUrl: string): Promise<Site> {
    let res: Site;
    const s = await JSDOM.fromURL(reqUrl); //timeout 설정이 불가능...
