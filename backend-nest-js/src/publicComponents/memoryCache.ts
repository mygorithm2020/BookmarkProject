...
export class ServerCache {
  ...
  static async setCategorys(categorys: Category[]) {
  ...
  static async getCategorys(): Promise<Category[]> {
    // 시간이 지났거나, 값이 없으면 => 하루마다
    if (
      !this.cachedCategorys ||
      !this.cachedCategorys.categorys ||
  ...
  static async setRecommendSites(sites: Site[]) {
    this.recommendSites = {
      lastDate: new Date(),
      sites: sites,
    };
  ...
  // 생일(나이), 성별, 관심 카테고리
  static async getRecommendSites(
    categorys?: Category[],
    birth?: string,
    gender?: string,
  ): Promise<Site[]> {
  ...
  static async getSessionId(sessionId: string): Promise<string> {
    let res = sessionId;
    ...
    // 세션 유지시간........하루...???
    const dif = (now.getTime() - date.getTime()) / (1000 * 60 * 60);
    if (dif > 1) {
  ...
  static async checkRestrictedViews(
    ...
    if (!this.restrictedViews) {
      ...
      this.restrictedViews.lastUpdate.getUTCHours() !== new Date().getUTCHours()
    ) {
