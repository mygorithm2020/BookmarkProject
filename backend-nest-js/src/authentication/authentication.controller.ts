...
export class AuthenticationController {
  ...
  //인증번호 발송
  @Post('/sendemail')
  async sendEmailAuth(@Body() auth: Authentication) {
    let result = {};
    const serviceRes = await this.authService.sendAuthEmail(auth.Email);
    if (serviceRes) {
      result = {
        Email: auth.Email,
      };
  ...
  async signIn(@Body() memberObj: Member, @Req() req: Request) {
    ...
    // 계정이 존재하는지 확인
    const member = await this.memberService.checkWithEmailPw(
      memberObj.MemEmail,
      memberObj.Password,
    );
  ...
  async refreshAccToken(@Req() req: Request) {
    ...
    if (
      ...
      !tokenObj.Token ||
      tokenObj.IP !== req.ip ||
      tokenObj.UserAgent !== req.headers['user-agent'] ||
      tokenObj.Origin != req.headers.origin
    ) {
      throw new HttpException(
        {
          errCode: 11,
  ...
  async checkAuth(@Body() auth: Authentication) {
    const result = await this.authService.checkAuthCode(
      auth.Email,
      auth.AuthCode,
    );
    if (!result) {
