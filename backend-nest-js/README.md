...
#### 오류 및 문제 해결
    ...
    AWS EC2 프리티어 SSH 연결 속도 및 DB 연결 속도 문제
        - 프리티어로 처음에 EC2 기본 VM을 생성했으나, SSH 접속 및 다른 작업이 너무 느린 문제 발생
        - 기본 지역이 유럽 스톡홀롬으로 설정되어 있었고, 서울 지역으로 변경 후 재 생성해서 해결
...
#### 배포
    ...
    마이그레이션 시 주의사항 : 이미지 파일 및 폴더, DB 데이터, + 로그파일
    ...
    AWS : 프리티어 EC2 배포
        - AWS 콘솔 사이트 방문
        - EC2 인스턴스 생성(ubuntu 22.04 64비트 x86) : 프리티어에 해당하는 스펙 설정
          - 키페어 : firstkey  (RSA, pem)
          - 네트워크 SSH, HTTPS, HTTP
        - 탄력적 IP(아마 고정IP) 생성 및 인스턴스와 연결
        - 보안규칙으로 3306 포트 및 추가 필요 포트 개방
        - 주의 사항!
          -  콘솔 우 상단 리전을 꼭 가까운 지역으로 설정하기, 그렇지 않으면 거리로 인해 네트워크상의 속도 저하가 심함
          -  프리티어의 경우 CPU와 메모리 사용량이 현저히 낮아(1core, 1GiB) 운영 중 문제가 생기는 경우가 많음
          -  일단 임시로 가상메모리 swap 방식으로 처리(https://repost.aws/ko/knowledge-center/ec2-memory-swap-file)
